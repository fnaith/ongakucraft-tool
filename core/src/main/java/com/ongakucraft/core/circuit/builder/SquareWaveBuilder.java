...
public final class SquareWaveBuilder extends CircuitBuilder {
    ...
    public static SquareWaveBuilder of(BlockDataset blockDataset, boolean turnRight, String floorPath, String stubPath) {
        return new SquareWaveBuilder(blockDataset, turnRight, floorPath, stubPath);
    }
    ...
    private SquareWaveBuilder(BlockDataset blockDataset, boolean turnRight,
    ...
    public void generate(Cursor cursor, List<List<Note>> sequenceList) {
        if (1 != sequenceList.size()) {
            throw new OcException("[SquareWaveBuilder][generate] size : %d", sequenceList.size());
        }
