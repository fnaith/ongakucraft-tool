...
public final class SquareWaveExtendedBuilder extends CircuitBuilder {
    ...
    public static SquareWaveExtendedBuilder of(BlockDataset blockDataset, boolean turnRight, String floorPath, String stubPath) {
        return new SquareWaveExtendedBuilder(blockDataset, turnRight, floorPath, stubPath);
    }
    ...
    private SquareWaveExtendedBuilder(BlockDataset blockDataset, boolean turnRight,
    ...
    public void generate(Cursor cursor, List<List<Note>> sequenceList) {
        if (sequenceList.isEmpty() || 2 < sequenceList.size()) {
            throw new OcException("[SquareWaveExtendedBuilder][generate] size : %d", sequenceList.size());
        }
