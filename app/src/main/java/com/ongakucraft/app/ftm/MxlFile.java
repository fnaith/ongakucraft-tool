...
//        mxlFile.check();
    ...
    private List<Integer> mxlDivisions4List;
    private List<Integer> mxlMeasureSizeList;
    private int mxlRepeatCount;
    private int lastEndingStop;

    // process per score
    private int mxlDivisions4;
    private int mxlMeasureSize;
    @Getter
    private Map<Integer, Map<String, List<FtmNote>>> mxlPartToVoiceToChannel;
    private Map<Integer, Map<String, Integer>> mxlPartToVoiceToStaff;
    // process per part
    private Map<String, List<FtmNote>> mxlVoiceToChannel;
    private Map<String, Integer> mxlVoiceToStaff;
    private Map<Integer, Boolean> mxlStaffToPedal;
    private int mxlRowIndex;
    private int mxlPrevRows;
    private Map<String, Queue<Note>> mxlVoiceToTieStartNoteQueue;
    private Map<String, Integer> mxlVoiceToTuplet;

    public MxlFile(String filePath) {
        this.scorePartwise = loadMxl(filePath);
    }

    public void check() {
        parseMxl(this::checkMxlScore, this::checkMxlPart, this::checkMxlMeasure);
    }

    private void parseMxl(Consumer<ScorePartwise> scoreFunc,
    ...
    private void parseMxlWithRepeat(Consumer<ScorePartwise> scoreFunc,
                                    BiConsumer<Integer, ScorePartwise.Part> partFunc,
                                    BiConsumer<int[], ScorePartwise.Part.Measure> measureFunc) {
        if (null != scoreFunc) {
            scoreFunc.accept(scorePartwise);
    ...
    private void checkMxlScore(ScorePartwise scorePartwise) {
        mxlDivisions4List = new ArrayList<>();
        mxlMeasureSizeList = new ArrayList<>();
        mxlRepeatCount = 0;
    }

    private void checkMxlPart(int p, ScorePartwise.Part part) {
    ...
    private void checkMxlMeasure(int[] p_m, ScorePartwise.Part.Measure measure) {
        for (final var noteOrBackupOrForward : measure.getNoteOrBackupOrForward()) {
            if (noteOrBackupOrForward instanceof final Barline barline) {
                if (null != barline.getRepeat()) {
                    final var repeat = barline.getRepeat();
    ...
    private BigDecimal getDuration(Note note) { // TODO replace all note.getDuration()
        final var duration = note.getDuration();
        if (null != duration) {
            return duration;
        }
    ...
    private String pitchToName(Note note) {
        final var pitch = note.getPitch();
    ...
    private boolean isSamePitch(FtmNote ftmNote, Note note) {
    ...
    public void printMxl(String filePath, boolean printMeasures) {
        final var mxlFile = new MxlFile(filePath);
        if (printMeasures) {
            mxlFile.parseMxl(MxlFile::printMxlScore, MxlFile::printMxlPart, MxlFile::printMxlMeasure);
        } else {
            mxlFile.parseMxl(MxlFile::printMxlScore, MxlFile::printMxlPart, null);
        }
    }

    public List<List<String>> groupMxlNoteNameByRow(String filePath, int measures, int divisions4) {
    ...
    private List<FtmNote> newMxlChannel() {
    ...
    private void processMxlScore(ScorePartwise scorePartwise) {
    ...
    private void processMxlPart(int p, ScorePartwise.Part part) {
    ...
    private void processMxlMeasure(int[] p_m, ScorePartwise.Part.Measure measure) {
        for (final var noteOrBackupOrForward : measure.getNoteOrBackupOrForward()) {
            if (noteOrBackupOrForward instanceof final Backup backup) {
                final var duration = backup.getDuration().intValue();
...
//        checkMxl(rootDirPath + "5th/Asu e no Kyoukaisen - Yukihana Lamy/Asu_e_no_Taisen_-_Yukihana_Lamy-clean.mxl"); TODO
//        checkMxl(rootDirPath + "5th/Lunch with me - Momosuzu Nene/Lunch_with_Me.mxl"); TODO
//        checkMxl(rootDirPath + "5th/Lunch with me - Momosuzu Nene/Lunch_with_me (1).mxl"); TODO
//        checkMxl(rootDirPath + "5th/Nenenenenenenene Daibakusou - Momosuzu Nene/.mxl"); TODO
...
//        checkMxl(rootDirPath + "5th/HOLOGRAM CIRCUS - Omaru Polka/HOLOGRAM_CIRCUS.mxl"); TODO
//        checkMxl(rootDirPath + "5th/HOLOGRAM CIRCUS - Omaru Polka/HOLOGRAM_CIRCUS_-_Omaru_Polka.mxl"); TODO
//        checkMxl(rootDirPath + "5th/Saikyoutic Polka/.mxl"); TODO
