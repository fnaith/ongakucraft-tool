...
//        GraphicUtils.writeImage(slitMap, String.format("%s/slitMap.png", inputDirPath));
    ...
    private static void otonoseKanadeMapAnimation(BlockDatasetVersion version, String inputDirPath, String outputDirPath) throws Exception {
/*
...
//        GraphicUtils.writeImage(slitMap, String.format("%s/slitMap.png", inputDirPath));
    ...
    public static Structure otonoseKanadeMapAnimationCircuit(BlockDatasetVersion version) throws Exception {
        final var frames = 64;
        final var framesPerEdge = 8;
...
//            NbtWriter.of(VERSION).write(structure, outputFilePath);
            ...
            final var inputDirPath = String.format("%s/input/otonose_kanade/frames", ROOT_DIR_PATH);
            final var outputDirPath = String.format("%s/%s/map/otonose_kanade", ROOT_DIR_PATH, VERSION.getMcVersion());
            otonoseKanadeMapAnimation(VERSION, inputDirPath, outputDirPath);

            final var structure = otonoseKanadeMapAnimationCircuit(VERSION);
            final var outputFilePath = String.format("%s/%s/structure/otonose_kanade-player.nbt", ROOT_DIR_PATH, VERSION.getMcVersion());
            NbtWriter.of(VERSION).write(structure, outputFilePath);
        } catch (Exception e) {
